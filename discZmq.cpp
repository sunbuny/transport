...
void transport::Node::SpinOnce()
{
  ...
  zmq::pollitem_t items[] = {
    ...
    { 0, (zmq_fd_t)this->bcastSock->sockDesc, ZMQ_POLLIN, 0 },
                                ...
                                std::string &_response)
{
  ...
  if (items[0].revents & ZMQ_POLLIN)
  {
    zmq::multipart_t *reply = new zmq::multipart_t(*this->srvRequester);

    std::string((char*)reply->popstr().c_str());
    std::string((char*)reply->popstr().c_str());
    _response = std::string((char*)reply->popstr().c_str());
...
void transport::Node::RecvDiscoveryUpdates()
{
  ...
  {
    std::cerr << "Exception receiving from the UDP socket: " << e.what() << std::endl;
    return;
  }
...
{
  zmq::multipart_t *msg = new zmq::multipart_t(*this->subscriber);
  if (this->verbose)
  {
  ...
  if (msg->size() != 3)
  {
    std::cerr << "Unexpected topic update. Expected 3 message parts but "
              << "received a message with " << msg->size() << std::endl;
    return;
  }

  // Read the DATA message
  std::string topic = std::string((char*)msg->popstr().c_str());
  msg->popstr(); // Sender
  std::string data = std::string((char*)msg->popstr().c_str());
...
{
  zmq::multipart_t *msg = new zmq::multipart_t(*this->srvReplier);
  if (this->verbose)
  {
  ...
  std::string topic = std::string((char*)msg->popstr().c_str());
  std::string sender = std::string((char*)msg->popstr().c_str());
  std::string data = std::string((char*)msg->popstr().c_str());
  ...
  if (this->topicsSrvs.AdvertisedByMe(topic))
  {
    ...
    reply.addstr(topic);
    reply.addstr(this->srvReplierEP);
...
{
  zmq::multipart_t *msg = new zmq::multipart_t(*this->srvRequester);
  if (this->verbose)
  {
  ...
  std::string topic = std::string((char*)msg->popstr().c_str());
  std::string((char*)msg->popstr().c_str());
  std::string response = std::string((char*)msg->popstr().c_str());
...
void transport::Node::SendPendingAsyncSrvCalls()
{
       ...
       it != this->topicsSrvs.GetTopicsInfo().end(); ++it)
  {
    ...
    while (this->topicsSrvs.PendingReqs(topic))
    {
      ...
      zmq::multipart_t msg;
      msg.addstr(topic);
      msg.addstr(this->srvRequesterEP);
      msg.addstr(data);
                                      ...
                                      const std::string &_address)
{
  ...
  {
    std::cerr << "Exception sending an ADV msg: " << e.what() << std::endl;
    delete[] buffer;
    return -1;
  }
...
int transport::Node::SendSubscribeMsg(uint8_t _type, const std::string &_topic)
{
  ...
  {
    std::cerr << "Exception sending a SUB msg: " << e.what() << std::endl;
    delete[] buffer;
    return -1;
  }
